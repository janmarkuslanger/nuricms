...
func (m *mockApikeyRepo) FindByID(id uint) (*model.Apikey, error) {
	args := m.Called(id)
	if obj := args.Get(0); obj != nil {
		return obj.(*model.Apikey), args.Error(1)
	}
...
func (m *mockApikeyRepo) List(page, pageSize int) ([]model.Apikey, int64, error) {
...
func TestApikeyService_List_And_FindByID(t *testing.T) {
	repo := new(mockApikeyRepo)
	svc := newTestService(repo)

	sample := []model.Apikey{{Model: gorm.Model{ID: 1}}, {Model: gorm.Model{ID: 2}}}

	repo.On("List", 2, 10).Return(sample, int64(2), nil)

	list, total, err := svc.List(2, 10)
	assert.NoError(t, err)
...
func TestApikeyService_DeleteByID(t *testing.T) {
	repo := new(mockApikeyRepo)
	svc := newTestService(repo)
